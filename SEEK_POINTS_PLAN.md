...
### Phase 3B: UI設計変更実装 (予定)
- [ ] **Step 3B.1**: シークポイントタブの選択楽曲ベース化
  - `get_current_track_seek_points()` → `get_selected_track_seek_points()`
  - ジャンプ機能削除（ボタン→ラベル表示に変更）
  - 編集・削除機能はそのまま維持
- [ ] **Step 3B.2**: 再生コントロール領域の拡張
  - 楽曲情報表示下にシークポイントリスト追加
  - クリック/ダブルクリックでジャンプ機能実装
  - 簡潔な表示形式（名前のみ、または名前+時間）
- [ ] **Step 3B.3**: 統合テスト・動作確認
  - 管理機能（シークポイントタブ）のテスト
  - 操作機能（再生コントロール）のテスト
  - 選択楽曲≠再生楽曲での動作確認

**🎯 Phase 3B完了条件**: 論理的に分離された直感的なシークポイントUI
...
│ ⏮ ↩ ▶ ⏹ ↪ ⏭   │
│ ♪ Artist - Title  │ ← 楽曲情報表示
│ ● ポイント1 02:45  │ ← ★ 新規: シークポイント操作
│ ● ポイント2 04:10  │ ← クリックでジャンプ
└────────────────────┘
┌─ シークポイント管理 ─┐
│ シークポイント追加    │ ← そのまま維持
└─────────────────────┘
